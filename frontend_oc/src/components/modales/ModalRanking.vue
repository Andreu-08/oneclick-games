  ...
  <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
    <div class="bg-white rounded-2xl shadow-xl p-6 w-full max-w-2xl border border-blue-100">
      ...
      <ul class="flex flex-col gap-2">
        <li v-for="(entry, index) in ranking" :key="entry.id"
          :class="[
      ...
      <div class="mt-5 text-center">
        <button @click="$emit('close')" class="w-full max-w-[200px] py-2 bg-blue-600 text-white rounded-full hover:bg-blue-700">
          Cerrar
        </button>
      </div>
    </div>
